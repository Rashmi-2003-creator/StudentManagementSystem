...
internal class Program
{
    ...
    private static void InsertMarksAndCalculateGPA()
    {
        ...
        using (SqlConnection connection = new(connectionString))
        {
            ...
            if (rowsAffected > 0)
            {
                CalculateAndSetGPA(connection, studentID, courseID);
    ...
    private static void CalculateAndSetGPA(SqlConnection connection, int studentID, int courseID)
    {
        using SqlCommand cmd = new("UPDATE Enrollment SET Grade = " +
            "CASE " +
            "WHEN Grade >= 90 THEN 0.9 " +
            "WHEN Grade >= 80 THEN 0.8 " +
            "WHEN Grade >= 70 THEN 0.7 " +
            "WHEN Grade >= 60 THEN 0.6 " +
            "WHEN Grade >= 50 THEN 0.5 " +
            "ELSE 0.0 " +
            "END " +
